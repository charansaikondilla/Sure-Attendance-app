...
function App() {
  ...
  const processAttendanceData = async (data) => {
    try {
      ...
      const result = await submitAttendance(uniqueStudents)
      ...
      const mockResponse = {
        present: data.flat().filter(student => student && student.trim() !== '').map(student => student.trim()),
        absentees: [],
        unknowns: [],
        totalProcessed: data.flat().filter(student => student && student.trim() !== '').length,
        error: 'Failed to connect to Google Sheets. Using local processing.'
      }
